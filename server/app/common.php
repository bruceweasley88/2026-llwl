...
{
    $salt = env('project.unique_identification', '1bfd');
...
{
    $list = [
        "route_distance_km" => "里程",


        'vehicle_vehicle_type' => '订单车辆类型',
        'vehicle_fuel' => '订单车辆油耗',
        "vehicle_start_price" => "订单车辆起步价",
        'vehicle_odometer_coefficient' => '订单车辆公里系数',
        "vehicle_size" => "订单车辆尺寸",

        'start_vehicle_vehicle_type' => '出发车辆类型',
        'start_vehicle_fuel' => '出发车辆油耗',
        "start_vehicle_start_price" => "出发车辆起步价",
        'start_vehicle_odometer_coefficient' => '出发车辆公里系数',
        "start_vehicle_size" => "出发车辆尺寸",

        "toll" => "高速费",
        "parking_fee" => "停车费",
        "route_store_count" => "门店数",
        'project_type_type_name' => '项目类型',
        'transport_company_name' => '运输公司',
        'route_fuel_price' => '基准油价',
        'amount' => '单价',
        'fuel_amount' => '加油量',
        'fuel_costs' => '加油费用',
    ...
    foreach ($list as $key => $value) {
        $formula = str_replace($value, '$' . $key, $formula);
        $data[$key] = $data[$key] ?? 0;
    }
    ...
    $formula = str_replace(',', '', $formula);
    $formula = str_replace('￥', '', $formula);
    $formula = str_replace('元', '', $formula);


    // 将数据对象转为数组
    $dataArray = is_array($data) ? $data : get_object_vars($data);
    ...
    $result = 0;
    try {
        eval('$result = round(' . $formula . ', 2, PHP_ROUND_HALF_DOWN);');
    } catch (ParseError $e) {
        return "公式错误" . $e->getMessage() . '|' . $formula;
    } catch (Throwable $e) {
        return "公式错误" . $e->getMessage() . '|' . $formula;
    }
