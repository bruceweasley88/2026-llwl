...
class DetailsProjectLists extends BaseAdminDataLists implements ListsSearchInterface
{
     ...
     * @date 2025/12/31 00:12
     */
    private function getCustomWhere(): array
    {
        $where = [];
        ...
        if (input('has_note')) {
            $hasNote = input('has_note') === '是';
            if ($hasNote) {
                // 有备注：note 不为空字符串且不为 NULL
                $where[] = ['la_details_project.note', '<>', ''];
            } else {
                // 无备注：note 为空字符串或 NULL
                $where[] = function($query) {
                    $query->where('la_details_project.note', '=', '')
                          ->whereOr('la_details_project.note', 'exp', 'IS NULL');
                };
            }
        }

        if (input('has_reimbursement')) {
            $hasReim = input('has_reimbursement') === '是';
            $reimFields = ['is_reim_other_expenses', 'is_reim_maintenance_cost',
                           'is_reim_fuel_costs', 'is_reim_parking_fee', 'is_reim_toll'];

            if ($hasReim) {
                // 有报销：至少一个字段为 1 (OR 逻辑)
                $where[] = function($query) use ($reimFields) {
                    foreach ($reimFields as $i => $field) {
                        if ($i === 0) {
                            $query->where('la_details_project.' . $field, '=', 1);
                        } else {
                            $query->whereOr('la_details_project.' . $field, '=', 1);
                        }
                    }
                };
            } else {
                // 无报销：所有字段都为 0 (AND 逻辑)
                foreach ($reimFields as $field) {
                    $where[] = ['la_details_project.' . $field, '=', 0];
                }
            }
        }
    ...
    public static function getDetailsProjectList($searchWhere = [], $where = [], $offset = 0, $limit = 10): array
    {
        ...
        foreach ($list as $key => $item) {
            ...
            if (empty($item['amount_formula_formula'])) {
                $list[$key]['amount'] = 0;
            } elseif ($item['c_amount'] > 0) {
                $list[$key]['amount'] = $item['c_amount'];
            } else {
            ...
            if (empty($item['commission_formula_formula'])) {
                $list[$key]['c_commission'] = 0;
            } elseif ($item['c_amount'] > 0) {
                $list[$key]['driver_commission'] = $item['c_commission'];
            } else {
    ...
    public function count(): int
    {
        ...
        return DetailsProject::where($this->searchWhere)
            ->where($where)
            ->count();
    }
}
