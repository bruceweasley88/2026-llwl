                ...
                <el-form-item label="公式" prop="formula">
                    ...
                    <div class="formula-hint mt-2">
                        ...
                        <div class="hint-example mt-2">
                            ...
                            <span class="text-blue-600">车辆起步价+里程*车辆公里系数+里程*基准油价*车辆油耗</span>
                        </div>
                    </div>
                </el-form-item>
...
<script lang="ts" setup name="amountFormulaEdit">
import type { FormInstance } from 'element-plus'
import Popup from '@/components/popup/index.vue'
import { apiAmountFormulaAdd, apiAmountFormulaEdit, apiAmountFormulaDetail } from '@/api/amount_formula'
...
const popupTitle = computed(() => {
    return mode.value == 'edit' ? '编辑单价公式' : '新增单价公式'
})
...
const getDetail = async (row: Record<string, any>) => {
    const data = await apiAmountFormulaDetail({
        id: row.id
    })
    setFormData(data)
}
...
const handleSubmit = async () => {
    ...
    const data = { ...formData, }
    mode.value == 'edit'
        ? await apiAmountFormulaEdit(data)
        : await apiAmountFormulaAdd(data)
